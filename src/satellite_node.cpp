...
int main(int argc, char** argv)
{
  ...
  ros::Publisher local_pos_pub = nh.advertise<geometry_msgs::PoseStamped>(
      "mavros_satellite/setpoint_position/local", 10);

  ros::Publisher local_vel_pub = nh.advertise<geometry_msgs::TwistStamped>(
      "mavros_satellite/setpoint_velocity/cmd_vel", 10);

  ros::Subscriber local_pos_sub = nh.subscribe<geometry_msgs::PoseStamped>(
      "mavros_satellite/local_position/pose", 10, position_cb);
  ...
  ros::Subscriber state_sub =
      nh.subscribe<mavros_msgs::State>("mavros_satellite/state", 10, state_cb);

  ros::ServiceClient arming_client =
      nh.serviceClient<mavros_msgs::CommandBool>("mavros_satellite/cmd/arming");

  ros::ServiceClient set_mode_client =
      nh.serviceClient<mavros_msgs::SetMode>("mavros_satellite/set_mode");

  // the setpoint publishing rate MUST be faster than 2Hz 아니면 OFFBOARD 취소 됨
  ros::Rate rate(20.0); // period 0.05 s

  // satellite_node의 mode 라는 para를 mode 라는 변수에 넣고 값은 -1
  nh.param("satellite_node/mode", mode, -1);
  ...
  pos_set.pose.position.x = satellite_x;
  pos_set.pose.position.y = satellite_y;
  pos_set.pose.position.z = satellite_z;
